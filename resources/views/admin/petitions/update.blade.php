...
@section('content')
    <div class="container">
        ...
        <form action="{{ route('admin.petitions.update', $petition->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control" id="description" name="description" rows="4" required>{{ old('description', $petition->description) }}</textarea>
            </div>

            <!-- Destinatario -->
            <div class="mb-3">
                <label for="addressee" class="form-label">Destinatario</label>
                <input type="text" class="form-control" id="addressee" name="addressee" value="{{ old('addressee', $petition->addressee) }}" required>
            </div>

            <!-- Firmantes -->
            <div class="mb-3">
                <label for="signatories" class="form-label">Firmantes</label>
                <input type="number" class="form-control" id="signatories" name="signatories" value="{{ old('signatories', $petition->signatories) }}" min="0">
            </div>
                ...
                <div class="d-flex flex-wrap gap-3">
                    @foreach($petition->files as $file)
                        <div class="position-relative">
                            <img src="{{ asset($file->file_path) }}" alt="{{ $file->name }}" width="150" class="img-thumbnail">
                            <form action="{{ route('admin.petitions.delete', $file->id) }}" method="POST" class="position-absolute top-0 end-0">
                                @csrf
                                @method('DELETE')
